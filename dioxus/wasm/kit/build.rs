// Build script for the kit crate
// -------------------------------------------------------------------------------------------------
...
// Build the icons rust files from the raw icon files
// -------------------------------------------------------------------------------------------------
...
fn make_icons(src_dir: &PathBuf) -> Vec<Icon> {
    ...
    {
        let name = entry.file_name().to_str().unwrap().trim_end_matches(".svg").to_upper_camel_case();
        ...
        icons.push(Icon {
            ...
            path: svg
                ...
                .collect::<Vec<_>>()
                .join(" "),
            clip_rule: svg.select(&path_sel).find_map(|e| e.value().attr("clip-rule")).map(|r| r.to_string()),
            fill_rule: svg.select(&path_sel).find_map(|e| e.value().attr("fill-rule")).map(|r| r.to_string()),
        });
    }
...
impl crate::icons::IconShape for Shape {
...
fn write_icons_file(icons: &[Icon], filename: &str) {
    ...
    let names = icons.iter().map(|i| i.name.as_str()).collect::<Vec<_>>().join(",\n");
    ...
    let paths = icons
        ...
        .map(|i| {
            ...
            .collect::<Vec<_>>()
            .join("\n");
            PATH_TEMPLATE.replace("{NAME}", &i.name).replace("{ATTRS}", attrs)
        })
        .collect::<Vec<_>>()
        .join("");

    let code =
        TEMPLATE.replace("{VIEWBOX}", &icons[0].viewbox).replace("{NAMES}", &names).replace("{PATHS}", &paths);
...
fn attr(name: &str, value: Option<&str>, indent: bool) -> Option<String> {
    value.map(|v| format!(r#"{}{}: "{}","#, if indent { "        " } else { "" }, name, v,))
}
