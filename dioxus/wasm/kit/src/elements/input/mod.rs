...
use crate::icons::Icon as IconElement;
...
fn validate_no_whitespace(val: &str) -> Option<ValidationError> {
    if val.contains(char::is_whitespace) {
        return Some("spaces not allowed".to_string());
    }
    None
}
...
fn validate_alphanumeric(
    val: &str, ignore_colon: bool, special_characters: Option<(SpecialCharsAction, Vec<char>)>,
    ...
    if let Some((action, chars)) = special_characters {
        ...
        if action == SpecialCharsAction::Block {
            special_chars_allowed =
                special_chars_allowed.iter().filter(|&&c| !chars.contains(&c)).cloned().collect();
        } else {
    ...
    if !val.chars().all(char::is_alphanumeric) {
        return Some("only alpha chars".to_string());
    }
...
pub fn validate_min_max(val: &str, min: Option<i32>, max: Option<i32>) -> Option<ValidationError> {
    ...
    if max > 0 && val.len() > max {
        return Some(format!("{} {} {} {}.", "Maximum of", max, "characters", "exceeded"));
    }
    ...
    if min > 0 && val.len() < min {
        return Some(format!("{} {} {}.", "Please enter at least", min, "characters"));
    }
...
pub fn validate(cx: &Scope<Props>, val: &str) -> Option<ValidationError> {
    ...
    if validation.alpha_numeric_only
        && validate_alphanumeric(val, validation.ignore_colons, validation.special_chars.clone()).is_some()
    {
...
pub fn Input<'a>(cx: Scope<'a, Props<'a>>) -> Element<'a> {
    ...
    let input_id =
        use_state(cx, || if cx.props.id.is_empty() { Uuid::new_v4().to_string() } else { cx.props.id.clone() });
