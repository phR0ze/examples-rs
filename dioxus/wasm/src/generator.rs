...
const KEYWORDS: &str = include_str!("../dist/assets/data/keywords.txt");
const SYLLABLES: &str = include_str!("../dist/assets/data/syllables.txt");
const YEW_CONTENT: &str = include_str!("../dist/assets/data/yew.txt");
...
impl Generator {
    ...
    pub fn human_name(&mut self) -> String {
        ...
        let first_name = SYLLABLES.split_whitespace().choose_multiple(&mut self.rng, n_syllables).join("");

        let n_syllables = self.rng.gen_range(SYLLABLES_MIN..SYLLABLES_MAX);
        let last_name = SYLLABLES.split_whitespace().choose_multiple(&mut self.rng, n_syllables).join("");
    ...
    pub fn keywords(&mut self) -> Vec<String> {
        ...
        KEYWORDS.split_whitespace().map(ToOwned::to_owned).choose_multiple(&mut self.rng, n_keywords)
    }
