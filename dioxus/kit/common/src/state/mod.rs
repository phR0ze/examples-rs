...
impl State {
    ...
    pub fn mutate(&mut self, action: Action) {
        ...
        match action {
            ...
            Action::AddNotification(kind, count) => {
                self.ui.notifications.increment(&self.configuration, kind, count, !self.ui.metadata.focused)
            },
            ...
            Action::AddToastNotification(notification) => {
                self.ui.toast_notifications.insert(Uuid::new_v4(), notification);
            },
            Action::DismissUpdate => {
                self.settings.update_dismissed = self.settings.update_available.take();
                self.ui.notifications.decrement(notifications::NotificationKind::Settings, 1);
            },
            // ===== Friends =====
            ...
            Action::MockSend(id, msg) => {
                ...
                m.set_sender(sender);
                m.set_value(msg);
                let m = ui_adapter::Message { inner: m, in_reply_to: None, key: Uuid::new_v4().to_string() };
    ...
    fn process_multipass_event(&mut self, event: MultiPassEvent) {
        match event {
            ...
            MultiPassEvent::FriendRequestReceived(identity) => {
                ...
                self.mutate(Action::AddNotification(notifications::NotificationKind::FriendRequest, 1));
        ...
        match event {
            RayGunEvent::ConversationCreated(chat) => {
                if !self.chats.in_sidebar.contains(&chat.inner.id) {
                    ...
                    self.identities.extend(chat.identities.iter().map(|ident| (ident.did_key(), ident.clone())));
                }
    ...
    fn process_message_event(&mut self, event: MessageEvent) {
        match event {
            MessageEvent::Received { conversation_id, message } => {
                ...
                self.mutate(Action::AddNotification(notifications::NotificationKind::Message, 1));

                // TODO: Get state available in this scope.
                // Dispatch notifications only when we're not already focused on the application.
                let notifications_enabled = self.configuration.notifications.messages_notifications;
                let should_play_sound =
                    self.ui.current_layout != Layout::Compose && self.configuration.audiovideo.message_sounds;
                let should_dispatch_notification = notifications_enabled && !self.ui.metadata.focused;
                ...
                if should_dispatch_notification {
                    ...
                    let text = match id {
                        Some(id) => format!("{} {}", id.username(), get_local_text("messages.user-sent-message"),),
            ...
            MessageEvent::Sent { conversation_id, message } => {
                // todo: don't load all the messages by default. if the user scrolled up, for example, this incoming message may not need to be fetched yet.
            ...
            MessageEvent::Edited { conversation_id, message } => {
                self.update_identity_status_hack(&message.inner.sender());
                if let Some(chat) = self.chats.all.get_mut(&conversation_id) {
                    if let Some(msg) = chat.messages.iter_mut().find(|msg| msg.inner.id() == message.inner.id()) {
                        *msg = message;
                    }
                }
            },
            MessageEvent::Deleted { conversation_id, message_id } => {
                if let Some(chat) = self.chats.all.get_mut(&conversation_id) {
            ...
            MessageEvent::TypingIndicator { conversation_id, participant } => {
...
impl State {
    pub fn mock(
        my_id: Identity, mut identities: HashMap<DID, Identity>, chats: chats::Chats, friends: friends::Friends,
        storage: Storage,
    ) -> Self {
        let id = my_id.did_key();
        identities.insert(my_id.did_key(), my_id);
        Self { id, route: Route { active: "/".into() }, storage, chats, friends, identities, ..Default::default() }
    }
    /// Saves the current state to disk.
    pub fn save(&self) -> Result<(), Box<dyn std::error::Error>> {
        let serialized = serde_json::to_string_pretty(self)?;
        let path = if STATIC_ARGS.use_mock { &STATIC_ARGS.mock_cache_path } else { &STATIC_ARGS.cache_path };
...
impl State {
    ...
    pub fn chats_favorites(&self) -> Vec<Chat> {
        self.chats.favorites.iter().filter_map(|did| self.chats.all.get(did)).cloned().collect()
    }
    pub fn chats_sidebar(&self) -> Vec<Chat> {
        self.chats.in_sidebar.iter().filter_map(|did| self.chats.all.get(did)).cloned().collect()
    }
    pub fn chat_participants(&self, chat: &Chat) -> Vec<Identity> {
        chat.participants.iter().filter_map(|did| self.identities.get(did)).cloned().collect()
    }
    pub fn set_chats(&mut self, chats: HashMap<Uuid, Chat>, identities: HashSet<Identity>) {
        ...
        self.identities.extend(identities.iter().map(|x| (x.did_key(), x.clone())));
    }
    fn add_msg_to_chat(&mut self, conversation_id: Uuid, message: ui_adapter::Message) {
        if let Some(chat) = self.chats.all.get_mut(&conversation_id) {
            ...
            if self.ui.current_layout != ui::Layout::Compose || self.chats.active != Some(conversation_id) {
                chat.unreads += 1;
            }
        }
    }
    ...
    pub fn active_chat_has_draft(&self) -> bool {
        self.get_active_chat().as_ref().and_then(|d| d.draft.as_ref()).map(|d| !d.is_empty()).unwrap_or(false)
    }
        ...
        self.get_active_chat()
            .map(|c| {
                if c.conversation_type == ConversationType::Direct {
                    return c.participants.iter().all(|e| e.eq(&self.did_key()) || self.has_friend_with_did(e));
                }
    ...
    pub fn clear_typing_indicator(&mut self, instant: Instant) -> bool {
        ...
        for conv_id in self.chats.in_sidebar.iter() {
            ...
            chat.typing_indicator.retain(|_id, time| instant - *time < Duration::from_secs(5));
    ...
    pub fn get_active_chat(&self) -> Option<Chat> {
        self.chats.active.and_then(|uuid| self.chats.all.get(&uuid).cloned())
    }
    pub fn get_active_media_chat(&self) -> Option<&Chat> {
        self.chats.active_media.and_then(|uuid| self.chats.all.get(&uuid))
    }
    ...
    pub fn get_chat_with_friend(&self, friend: DID) -> Option<Chat> {
        self.chats
            .all
            .values()
            .find(|chat| chat.conversation_type == ConversationType::Direct && chat.participants.contains(&friend))
            .cloned()
    }
    // assumes the messages are sorted by most recent to oldest
    pub fn prepend_messages_to_chat(&mut self, conversation_id: Uuid, mut messages: Vec<ui_adapter::Message>) {
        if let Some(chat) = self.chats.all.get_mut(&conversation_id) {
...
impl State {
    ...
    pub fn get_friends_by_first_letter(friends: HashMap<DID, Identity>) -> BTreeMap<char, Vec<Identity>> {
        ...
        for (_, friend) in friends {
            let first_letter =
                friend.username().chars().next().expect("all friends should have a username").to_ascii_lowercase();

            friends_by_first_letter.entry(first_letter).or_insert_with(Vec::new).push(friend.clone());
        }

        for (_, list) in friends_by_first_letter.iter_mut() {
            list.sort_by(|a, b| {
                a.username().cmp(&b.username()).then(a.did_key().to_string().cmp(&b.did_key().to_string()))
            })
        }
    ...
    fn remove_friend(&mut self, did: &DID) {
        ...
        let direct_chat = all_chats.values().find(|chat| {
            chat.conversation_type == ConversationType::Direct
                && chat.participants.iter().any(|participant| participant == did)
        });
...
impl State {
    pub fn blocked_fr_identities(&self) -> Vec<Identity> {
        self.friends.blocked.iter().filter_map(|did| self.identities.get(did)).cloned().collect()
    }
    pub fn friend_identities(&self) -> Vec<Identity> {
        self.friends.all.iter().filter_map(|did| self.identities.get(did)).cloned().collect()
    }
    pub fn get_identities(&self, ids: &[DID]) -> Vec<Identity> {
        ids.iter().filter_map(|id| self.identities.get(id)).cloned().collect()
    }
    pub fn get_identity(&self, did: &DID) -> Option<Identity> {
        self.identities.get(did).cloned()
    }
    pub fn get_own_identity(&self) -> Identity {
        self.identities.get(&self.did_key()).cloned().unwrap_or_default()
    }
    pub fn incoming_fr_identities(&self) -> Vec<Identity> {
        self.friends.incoming_requests.iter().filter_map(|did| self.identities.get(did)).cloned().collect()
    }
    /// Getters
    ...
    pub fn outgoing_fr_identities(&self) -> Vec<Identity> {
        self.friends.outgoing_requests.iter().filter_map(|did| self.identities.get(did)).cloned().collect()
    }
    ...
    pub fn search_group_chats(&self, name_prefix: &str) -> Vec<identity_search_result::Entry> {
        let get_display_name = |chat: &Chat| -> String {
            let names: Vec<_> =
                chat.participants.iter().filter_map(|id| self.identities.get(id)).map(|x| x.username()).collect();
        ...
        self.chats
            ...
            .filter(|(_k, v)| {
                let names: Vec<_> =
                    v.participants.iter().filter_map(|id| self.identities.get(id)).map(|x| x.username()).collect();
    ...
    pub fn profile_picture(&self) -> String {
        self.identities.get(&self.did_key()).map(|x| x.profile_picture()).unwrap_or_default()
    }

    pub fn profile_banner(&self) -> String {
        self.identities.get(&self.did_key()).map(|x| x.profile_banner()).unwrap_or_default()
    }
    pub fn join_usernames(identities: &[Identity]) -> String {
        identities.iter().map(|x| x.username()).collect::<Vec<String>>().join(", ")
    }
    ...
    pub fn remove_self(&self, identities: &[Identity]) -> Vec<Identity> {
        identities.iter().filter(|x| x.did_key() != self.did_key()).cloned().collect()
    }
    pub fn status_message(&self) -> Option<String> {
        self.identities.get(&self.did_key()).and_then(|x| x.status_message())
    }
    pub fn username(&self) -> String {
        self.identities.get(&self.did_key()).map(|x| x.username()).unwrap_or_default()
    }
}
...
pub mod identity_search_result {
    ...
    impl Entry {
        pub fn from_identity(name: String, did: DID) -> Self {
            Self { display_name: name, id: Identifier::Did(did) }
        }

        pub fn from_chat(name: String, id: Uuid) -> Self {
            Self { display_name: name, id: Identifier::Uuid(id) }
        }
    }
}
...
impl<'a> MessageGroup<'a> {
    pub fn new(sender: DID, my_did: &DID) -> Self {
        Self { remote: sender != *my_did, sender, messages: vec![] }
    }
}
...
pub fn group_messages<'a>(
    my_did: DID, num: usize, when_to_fetch_more: usize, input: &'a VecDeque<ui_adapter::Message>,
) -> Vec<MessageGroup<'a>> {
    ...
    for msg in iter {
        ...
        let g = GroupedMessage { message: msg, is_first: true, is_last: true, should_fetch_more: need_more() };
        grp.messages.push(g);
        messages.push(grp);
    }
