...
pub fn Sidebar(cx: Scope<Props>) -> Element {
            ...
            while let Some(cmd) = rx.next().await {
                match cmd {
                    MessagesCommand::CreateConversation { recipient } => {
                        ...
                        if let Err(e) =
                            warp_cmd_tx.send(WarpCmd::RayGun(RayGunCmd::CreateConversation { recipient, rsp: tx }))
                        {
                    ...
                    MessagesCommand::DeleteConversation { conv_id } => {
                        *show_delete_conversation.write_silent() = false;
                        ...
                        if let Err(e) =
                            warp_cmd_tx.send(WarpCmd::RayGun(RayGunCmd::DeleteConversation { conv_id, rsp: tx }))
                        {
                            log::error!("failed to send warp command: {}", e);
                            continue;
                        }
    ...
    let (sidebar_chats, favorites, active_media_chat) = if state.read().chats().initialized {
        (state.read().chats_sidebar(), state.read().chats_favorites(), state.read().get_active_chat())
    } else {
        (vec![], vec![], None)
    };
