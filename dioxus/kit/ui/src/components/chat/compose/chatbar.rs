...
pub fn get_chatbar<'a>(cx: &'a Scoped<'a, super::ComposeProps>) -> Element<'a> {
    ...
    let msg_ch = use_coroutine(cx, |mut rx: UnboundedReceiver<(Vec<String>, Uuid, Option<Uuid>)>| {
            ...
            while let Some((msg, conv_id, reply)) = rx.next().await {
                ...
                let cmd = match reply {
                    Some(reply_to) => {
                        ...
                        RayGunCmd::Reply { conv_id, reply_to, msg, attachments, rsp: tx }
                    },
                    None => {
                        let attachments = files_to_upload.current().to_vec();
                        RayGunCmd::SendMessage { conv_id, msg, attachments, rsp: tx }
                    },
                };
    ...
    let local_typing_ch = use_coroutine(cx, |mut rx: UnboundedReceiver<TypingIndicator>| {
        ...
        async move {
            ...
            let send_typing_indicator = |conv_id| async move {
                ...
                if let Err(e) = warp_cmd_tx.send(WarpCmd::RayGun(RayGunCmd::SendEvent { conv_id, event, rsp: tx }))
                {
                    log::error!("failed to send warp command: {}", e);
    ...
    let submit_fn = move || {
        ...
        if let Some(id) = active_chat_id {
            state.write().mutate(Action::SetChatDraft(id, String::new()));
        }
        ...
        with_file_upload: cx.render(rsx!(Button {
            ...
            onpress: move |_| {
                ...
                if let Some(new_files) =
                    FileDialog::new().set_directory(dirs::home_dir().unwrap_or_default()).pick_files()
                {
    ...
    let (platform, status, profile_picture) = match users_typing.first() {
        ...
        None => (identity::Platform::Unknown, IdentityStatus::Online, String::new()),
    };
...
fn Attachments<'a>(cx: Scope<'a, AttachmentProps>) -> Element<'a> {
    ...
    let attachments =
        cx.render(rsx!(cx.props.files.current().iter().map(|x| x.to_string_lossy().to_string()).map(
            |file_name| {
                rsx!(FileEmbed {
    ...
    cx.render(rsx!(div { id: "compose-attachments", aria_label: "compose-attachments", attachments }))
}
