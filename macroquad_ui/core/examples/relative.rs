...
async fn main() {
    ...
    loop {
        ...
        let size = Size::Percent(0.45, 0.25).relative(Size::screen());
        let pos = Position::Center(None).relative(size, Size::screen(), None);
        widgets::Button::new("").size(size).position(pos).ui(&mut *root_ui());
