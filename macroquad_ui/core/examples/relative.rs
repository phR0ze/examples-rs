...
async fn main() {
    ...
    loop {
        ...
        let size1 = Size::Percent(0.95, 0.40).relative(Size::screen());
        let pos1 = Position::CenterTop(rect(0., 0., 10., 0.)).relative(size1, Size::screen(), None);
        widgets::Button::new("").size(size1).position(pos1).ui(&mut *root_ui());
        root_ui().pop_skin();

        root_ui().push_skin(&blue_skin);
        let size2 = Size::Percent(0.75, 0.75).relative(size1);
        ...
        let size3 = Size::Percent(0.75, 0.75).relative(size2);
        ...
        let size4 = Size::Percent(0.75, 0.75).relative(size3);
        ...
        let size5 = Size::Percent(0.95, 0.40).relative(Size::screen());
        let pos5 = Position::CenterBottom(rect(0., 0., 0., 10.)).relative(size5, Size::screen(), None);
        widgets::Group::new(hash!(), size5).position(pos5).ui(&mut *root_ui(), |ui| {
            ui.push_skin(&gray_skin);
            let size6 = Size::Percent(1., 1.).relative(size5);
            ...
            let size7 = Size::Percent(0.75, 0.75).relative(size5);
            ...
            let size8 = Size::Percent(0.75, 0.75).relative(size7);
            ...
            let size9 = Size::Percent(0.75, 0.75).relative(size8);
